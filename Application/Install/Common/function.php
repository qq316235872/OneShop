...
function register_administrator($db, $prefix, $admin, $auth){
    ...
    $sql = "INSERT INTO `[PREFIX]ucenter_member` VALUES " .
           "('1', '[NAME]', '[PASS]', '[EMAIL]', '[MOBILE]', '[TIME]', '[IP]', 0, 0, '[TIME]', '1')";
    $password = user_md5($admin['password'], $auth);
    $sql = str_replace(
        array('[PREFIX]', '[NAME]', '[PASS]', '[EMAIL]', '[MOBILE]', '[TIME]', '[IP]'),
        array($prefix, $admin['username'], $password, $admin['email'], $admin['mobile'],NOW_TIME, get_client_ip(1)),
        $sql);
    //执行sql
    $db->execute($sql);
