...
class UserController extends HomeController {
	...
	/* 修改头像 */
        ...
        public function updateAvatar(){
             $head_pic_id =  $_REQUEST['head_pic_id'];
             //解析crop参数
             $crop 	= $_REQUEST['crop'];
             $crop 	= explode(',', $crop);
             $x 		= $crop[0];
             ...
             $path = '.'.get_cover( $head_pic_id,'path' );
             ...
             D('Picture')->where( array('id'=>$head_pic_id) )->save($file);

             //将此头像图片ID更新到用户记录表中
             echo D('Member')->where( array('uid'=>session('user_auth.uid')) )->setField('head_pic_id',$head_pic_id);
         }
