...
class SeoRuleModel extends Model
{
    ...
    {
        //查询缓存，如果已有，则直接返回
        $cacheKey = "os_seo_meta_{$module}_{$controller}_{$action}";
           $cache = S($cacheKey);
           if($cache !== false) {
               return $cache;
           }
        ...
        foreach ($rules as $e) {
            ...
            if ($e['module'] && $e['controller'] && $e['action']) {
                $need_seo = 0;
            }
    ...
    private function getRelatedRules($module, $controller, $action)
    {
        ...
        //查询与当前页面相关的SEO规则
        $map = array();
        $map['_string'] = "(module='' or module='$module') and (controller='' or controller='$controller') and (action='' or action='$action') and status=1";
