...
class AlipayNotify {
    ...
    //获取解析后的数组数组
    function get_out_trade_no($data){
    	$data = rsaDecrypt($data, $this->alipay_config['private_key_path']);  
	...
	function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			$_POST['trade_status'] = $_GET['trade_status'] = $doc->getElementsByTagName( "trade_status" )->item(0)->nodeValue;
